...
void RandomSamplingSoftmaxLossLayer<Dtype>::LayerSetUp(
  ...
  epsilon = this->layer_param_.rss_loss_param().epsilon();
  capitals_k = this->layer_param_.rss_loss_param().capitals_k();
...
void RandomSamplingSoftmaxLossLayer<Dtype>::Reshape(
  ...
  // refer to dimension of feature
  channels_ = bottom[0]->shape(softmax_axis_);
  // refer to 1
  inner_num_ = bottom[0]->count(softmax_axis_ + 1);
  // num of feature equals to labels
  CHECK_EQ(outer_num_ * inner_num_, bottom[1]->count())
      << "Number of labels must match number of predictions; "
      << "e.g., if softmax axis == 1 and prediction shape is (N, C, H, W), "
      << "label count (number of labels) must be N*H*W, "
      << "with integer values in {0, 1, ..., C-1}.";
  // channels must be larger than rss_num
...
void RandomSamplingSoftmaxLossLayer<Dtype>::random_sampling(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; ++j) {
      ...
      // label_value is bg_value
      if (label_value != bg_label) {
        rss_index_data[i * dim_rss + k * inner_num_ + j] = (Dtype)bg_label;
        k++;
      }
      // choose the index, and then get data according to the index
...
void RandomSamplingSoftmaxLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      loss -= ((Dtype)1.0 - epsilon + epsilon / capitals_k)*log(std::max(prob[i * dim + j], Dtype(FLT_MIN)));
      for(int k = 1; k < dim; k++){
        if(prob[i * dim + k * inner_num_ + j] > Dtype(FLT_MIN)){
          loss -= epsilon / capitals_k * log(prob[i * dim + k * inner_num_ + j]);
        }
      }
      ++count;
    }
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        for (int k = 0; k < rss_num_; ++k) {
          ...
          if(index != 0){
            if(prob[index] > Dtype(FLT_MIN)){
              bottom_diff[i * dim + c * inner_num_ + j] -= epsilon/capitals_k;
            }
          }
        }
